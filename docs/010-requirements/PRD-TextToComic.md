...
### Goals
- Hoàn thiện luồng end-to-end: nhập text -> tạo storyboard -> chỉnh panel -> tạo ảnh -> chỉnh speech bubble -> export.
...
- Nếu image backend không hỗ trợ reference, hệ thống vẫn dùng mô tả nhân vật trong prompt.
...
- UI hiển thị trạng thái `PENDING`, `GENERATING`, `SUCCESS`, `ERROR`.
